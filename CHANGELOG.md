# Changelog

앞으로 **바뀐 것만** 짧게 적습니다 (날짜 + 불릿).

## Unreleased

- 예외 앱 목록에 **보호 앱 먼저 / 비보호 앱 먼저 / 이름순** 정렬을 추가하고 마지막 선택을 저장.
- 실행 중인 앱을 우클릭해 그 앱과 자식·헬퍼만 추가 확인 없이 강제 종료하는 기능 추가.
- 앱 안에서 한국어·English·日本語를 직접 고를 수 있게 하고, 메인 경고창·설정·결과 문구를 즉시 현지화.

## 3.0.0

- 마케팅 **3.0.0**, 빌드 **18**.
- 종료 범위를 같은 UID 전체에서 **실행 중인 앱과 자식 프로세스** 중심으로 변경.
- 검색·필터·아이콘·자동 저장을 갖춘 **예외 앱** 화면 추가. 첫 실행 때 로그인 앱을 기본 예외로 한 번만 저장.
- 다죽이기는 사용자 예외를 무시하고, 적당히 죽이기는 앱 단위 예외를 헬퍼까지 함께 적용.
- 일치하는 서드파티 LaunchAgent를 현재 GUI 세션에서 내려 자동 재실행을 막음. 성공 시 killeverybody도 바로 정상 종료.
- 로그인 앱 조회를 관리자 암호를 요구하는 `sfltool`에서 읽기 전용 SharedFileList로 바꿔 실행 때마다 뜨던 권한 창 제거.
- Sparkle 2.9.5로 갱신. 실행 직후 백그라운드 확인·자동 다운로드/설치를 켜고, 릴리스 CI는 공식 `generate_appcast`로 GitHub Release 피드를 생성·검증.

## 2.2.0

- 마케팅 **2.2.0**, 빌드 **17**. **릴리즈 DMG**에 터미널용 **`killeverybody-cli`** 포함(앱과 동일한 후보 수집; 기본 `--dry-run`).
- GitHub Actions 릴리즈: Release 빌드 후 CLI **dry-run** 스모크, DMG 스테이징에 CLI 복사.

## 2.0.5

- 마케팅 **2.0.5**, 빌드 **15**. 메인 창 없이 순수 `NSAlert`만 띄우도록 재설계(창 백그라운드 완전 제거).
- `runModal`로 인한 Sparkle 창·설정 창 먹통 버그 수정.
- 앱 실행 중에 아이콘을 다시 누르면(또는 Finder에서 실행 시) 업데이트를 확인.
- Sparkle: `SUPublicEDKey` 누락 시 Console/log stream에 명시적 오류(Missing) 기록.

## 2.0.4

- 마케팅 **2.0.4**, 빌드 **14**. 메인 「다 죽일까요?」를 **시스템 `NSAlert` 시트**로 표시(커스텀 카드 UI 제거).
- Sparkle: `./scripts/verify-sparkle-appcast.sh`로 릴리즈 appcast의 `edSignature`·`length` 존재 여부를 빠르게 확인(CONTTRIBUTING 절차에 링크).

## 2.0.3

- 마케팅 **2.0.3**, 빌드 **13**. 커밋 메시지 검사에서 `Release 2.0.x` 형 제목 허용.

## 2.0.2

- Sparkle **EdDSA 키 재발급**: `INFOPLIST_KEY_SUPublicEDKey` 갱신, GitHub Actions secret `SPARKLE_PRIVATE_KEY`를 짝 맞는 시드로 설정. 빌드 번호 12. (업데이트 확인 EdDSA 오류 해소용 릴리즈)

## 2.0.0

- 마케팅 버전 **2.0.0**.
- README·SPEC에 영감 출처 명시: 키플러(kippler)의 Windows용 **다죽여**(AllKill) — [http://kippler.com/allkill/](http://kippler.com/allkill/).
- CI/Release: SPM `Sparkle.framework` 임베드 스크립트 — `BUILD_DIR` find 보강, 앱 번들 존재 확인, 빌드 페이즈를 링크 직후(Resources 앞)로 이동. 빌드 번호 10.
- 메인 UI: 앱 아이콘·회색 카드·캡슐형 버튼만(부가 설명 제거). 종료 후 피드백은 SIGKILL 일부 실패 시에만 알림.
- Sparkle: `SPUUpdaterDelegate`로 OSLog(`category: Sparkle`) 기록; CONTRIBUTING·스모크에 업데이터 실패 점검 절차.

## 1.3.0

- README 한국어·영어·일본어 분리 및 상호 링크(`README.md`, `README.en.md`, `README.ja.md`).
- 메인 UI를 **「다 죽일까요?」** 다이얼로그로 단순화(다죽이기 / 적당히 죽이기 / 종료, 확인 알림). 설정은 메뉴 ⌘,만.
- Sparkle: `Sparkle.framework`를 앱 번들에 Embed, 업데이터 초기화 시점 정리.

## 1.2.0

- Sparkle 자동 업데이트(피드: GitHub Release `appcast.xml`, 메뉴 「업데이트 확인…」).

## 1.1.1

- 앱 표시명·제품명·`.app` 이름 `killeverybody` 통일, 아이콘·표시명 Info 키 명시.
- 정책 보내기 기본 파일명 `killeverybody-policy.json`, `CHANGELOG.md` 추가.
