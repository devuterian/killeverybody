...
| **예외 앱 설정…** | 아이콘이 있는 앱 목록을 이름·번들 ID·경로로 검색하고 체크합니다. 필터와 저장되는 정렬 메뉴도 있어요. |
| **이 앱 강제 종료** | 예외 목록에서 실행 중인 앱을 우클릭하면 그 앱과 자식·헬퍼만 추가 확인 없이 종료합니다. 예외 체크는 그대로 둡니다. |
| **한국어·English·日本語** | 고급 설정에서 언어를 직접 고르면 메인 경고창과 설정 UI에 바로 적용됩니다. |
...
3. 보호할 앱을 남기려면 먼저 **예외 앱 설정…**에서 체크한 뒤 **적당히 죽이기**를 누릅니다.
4. 앱만 닫으려면 **종료**를 누르거나 `Esc`를 누릅니다.

### 적당히 죽이기에서 살아남을 앱

**예외 앱 설정…**을 누르면 별도 설정 창이 열립니다.

- 설치된 앱과 실행 중인 앱을 아이콘과 함께 보여 줍니다.
- 앱 이름, 번들 ID, 경로로 검색할 수 있습니다.
- 보호 앱 먼저(기본)·비보호 앱 먼저·이름순으로 정렬할 수 있고 마지막 선택을 기억합니다.
- 실행 중인 앱을 우클릭해 **이 앱 강제 종료**를 고르면 해당 앱과 자식·헬퍼만 바로 종료합니다. 성공하면 실행 상태만 조용히 갱신하고, 실패했을 때만 알려 줍니다.
- 첫 실행 때 macOS 로그인 앱을 기본 예외로 한 번만 체크합니다. 이 조회 때문에 관리자 암호를 묻지 않습니다.
- 체크를 바꾸면 바로 저장됩니다.
- **고급** 탭에서는 한국어·English·日本語를 고르고, 번들 ID를 직접 넣거나 메뉴 막대 앱으로 취급할 번들을 추가할 수 있습니다.
